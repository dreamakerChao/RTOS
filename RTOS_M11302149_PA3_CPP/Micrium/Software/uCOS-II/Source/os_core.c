...
INT8U GetEffectiveCeilingPriority(OS_TCB* ptcb, INT8U ceiling_raw, INT8U res_index) {
    INT8U ceiling = ceiling_raw - res_index;
    return (ptcb->OSTCBPrio > ceiling) ? ceiling : ptcb->OSTCBPrio;
}

INT8U GetCurrentEffectivePriority(OS_TCB* ptcb) {
    INT8U eff_prio = ptcb->OSTCBPrio;

    if (ptcb->holding_r1) {
        INT8U ceiling = R1_CEILING_PRIORITY - 1;
        if (ceiling < eff_prio) {
            eff_prio = ceiling;
        }
    }
    ...
    if (ptcb->holding_r2) {
        INT8U ceiling = R2_CEILING_PRIORITY - 2;
        if (ceiling < eff_prio) {
            eff_prio = ceiling;
        }
    }
